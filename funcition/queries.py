...
end_date = "2023-09-04 00:00:00"
getAllCabezaComprobantesDistriPPAL = f"""SELECT c.TIPOCOMPROBANTE, c.NUMEROCOMPROBANTE, c.CODIGOCLIENTE, c.FECHACOMPROBANTE, c.PORCIVA1, c.iva1, c.TOTAL, c.pagado, c.TIPOIVA, c.COMPRA, c.EXENTO  FROM CABEZACOMPROBANTES c WHERE  c.FECHACOMPROBANTE BETWEEN '{start_date}' AND '{end_date}' AND c.TIPOCOMPROBANTE IN ('FA', 'FB', 'NCA', 'NCB')"""
getAllCuerpoComprobantesDistriPPAL = f"""SELECT c.TIPOCOMPROBANTE, c.NUMEROCOMPROBANTE, c.LINEA, c.CODIGOARTICULO, c.PRECIOTOTAL, c.FECHAMODIFICACION, c.CODIGOARTICULO, c.CODIGOPARTICULAR, c.costoventa FROM cuerpocomprobantes c WHERE c.FECHAMODIFICACION BETWEEN '{start_date}' AND '{end_date}' AND c.TIPOCOMPROBANTE IN ('FA', 'FB', 'NCA', 'NCB')"""

# VENTAS DE DISTRI DS
getCabezaComprobantesDistriDS = f"""SELECT c.TIPOCOMPROBANTE, c.NUMEROCOMPROBANTE, c.CODIGOCLIENTE, c.FECHACOMPROBANTE, c.PORCIVA1, c.iva1, c.TOTAL, c.pagado, c.TIPOIVA, c.COMPRA, c.EXENTO  FROM CABEZACOMPROBANTES c WHERE  c.FECHACOMPROBANTE BETWEEN '{start_date}' AND '{end_date}' AND c.TIPOCOMPROBANTE IN ('FA', 'FB', 'NCA', 'NCB')"""
...
getCabezaComprobantesDimesPPAL = f"""SELECT c.TIPOCOMPROBANTE, c.NUMEROCOMPROBANTE, c.CODIGOCLIENTE, c.FECHACOMPROBANTE, c.PORCIVA1, c.iva1, c.TOTAL, c.pagado, c.TIPOIVA, c.COMPRA, c.EXENTO  FROM CABEZACOMPROBANTES c WHERE  c.FECHACOMPROBANTE BETWEEN '{start_date}' AND '{end_date}' AND c.TIPOCOMPROBANTE IN ('FA', 'FB', 'NCA', 'NCB')"""
getCuerpoComprobantesDimesPPAL = f"""SELECT c.TIPOCOMPROBANTE, c.NUMEROCOMPROBANTE, c.LINEA, c.CODIGOARTICULO, c.PRECIOTOTAL, c.FECHAMODIFICACION, c.CODIGOARTICULO, c.CODIGOPARTICULAR, c.costoventa FROM cuerpocomprobantes c WHERE c.FECHAMODIFICACION BETWEEN '{start_date}' AND '{end_date}' AND c.TIPOCOMPROBANTE IN ('FA', 'FB', 'NCA', 'NCB')"""

# VENTAS DE DIMES DS
getCabezaComprobantesDimesDS = f"""SELECT c.TIPOCOMPROBANTE, c.NUMEROCOMPROBANTE, c.CODIGOCLIENTE, c.FECHACOMPROBANTE, c.PORCIVA1, c.iva1, c.TOTAL, c.pagado, c.TIPOIVA, c.COMPRA, c.EXENTO  FROM CABEZACOMPROBANTES c WHERE  c.FECHACOMPROBANTE BETWEEN '{start_date}' AND '{end_date}' AND c.TIPOCOMPROBANTE IN ('FA', 'FB', 'NCA', 'NCB')"""
