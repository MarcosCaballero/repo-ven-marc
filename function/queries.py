...
def getDataPreviaCant(head, body, datesQuery, date, ppv, ppc, period='todo'):
     return f"""
            SELECT cd.CODIGOPARTICULAR, cd.RAZONSOCIAL, cdp.linea, md.CODIGOMARCA, cd.codigocliente, replace(cd.bonificacion, '.',',') as 'BONIFICACION', {ppv * 100} as 'PPV', {ppc * 100} as 'PPC', md.DESCRIPCION, {datesQuery}, 
...
# VENTAS DE DIMES DS
getCabezaComprobantesDimesDS = f"""SELECT c.TIPOCOMPROBANTE, c.NUMEROCOMPROBANTE, c.CODIGOCLIENTE, c.FECHACOMPROBANTE, c.PORCIVA1, c.iva1, c.TOTAL, c.pagado, c.TIPOIVA, c.COMPRA, c.EXENTO, c.DESCUENTOPORCENTAJE, c.DESCUENTOMONTO  FROM CABEZACOMPROBANTES c WHERE  c.FECHACOMPROBANTE BETWEEN '{start_date}' AND CURRENT_TIMESTAMP AND c.TIPOCOMPROBANTE IN ('FA', 'FB', 'NCA', 'NCB')"""
